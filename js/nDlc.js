...
// if(!user_token() && getUrlParam('code')=='' && location.href.indexOf('share_register')==-1){
//     location.replace("https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx96d1ab0d3a399299&redirect_uri="+window.location.href+"&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect");
// }
// if(!user_token() && getUrlParam('code')){
//     dlc_request('',{'HTTP_API':'vv/usercenter/api/user/third','code':getUrlParam('code'),'platform':'wechat'},function(res){
//         console.log(res);
//         if(res.code == 1){
//             user_save(res.data.userinfo.token);
//             localStorage.setItem('openid',res.data.thirdinfo.openid);
//             //location.reload();
//         }
//     },false);
// }
...
function dlc_request(method, data, cb, async, type) {
    var data = data || {};
    data.token = data.token ? data.token : user_token();
